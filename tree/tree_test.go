...
func validTree(t *testing.T, tree *Tree, tests []*entities.Msg) bool {
	...
	if len(msgMap) > 0 { //D:"aaaa_bbbb_c_uxviyl_r_ZgKhkc" ParentID:"aaaa_bbbb_c_uxviyl_r_ir2U4H"
...
func validNode(t *testing.T, msgMap map[string]*entities.Msg, node *node) bool {
	...
	if node.ParentID != msgMap[node.ID].ParentID {
		t.Error(`node is not under the proper parent`)
		return false
	}
